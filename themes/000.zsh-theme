...
prompt_context() {
  if [[ "$USERNAME" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    prompt_segment 32 15 " 🌸 %(!.%{%F{yellow}%}.)%n "
  fi
}
...
prompt_git() {
  ...
  ref=$(git symbolic-ref HEAD 2> /dev/null) || ref="➦ DETACHED:$(git rev-parse --short HEAD 2> /dev/null)"
  if [[ -n $dirty ]]; then
    prompt_segment 226 black
  else
    prompt_segment 156 $CURRENT_FG
  fi
  ...
  vcs_info
  print -n " ${${ref:gs/%/%%}/refs\/heads\//$PL_BRANCH_CHAR }${vcs_info_msg_0_%% }${mode}"
  fi
}
